...
public class StreamOperations {
    ...
    public long countMarriedPeople() {
        ...
        return persons.stream().filter(Person::isMarried).count();
    }
    ...
    public String retrieveLanguage(String name, String surname) {
        ...
        return persons.stream().filter(person -> person.getName().equals(name))
                ...
                .findFirst().map(Person::getLanguage).orElse(UNKNOWN_LANGUAGE);
    }

    // TODO Use sorted and replace specific comparator with Comparator.comparing
    // and thenComparing
    // TODO with static methods. Use limit method and collect with Collectors to
    // generate new List
    public List<Person> firstTenPersons() {
        ...
        return persons
                .stream()
                .sorted(Comparator.comparing(Person::getSurname).thenComparing(
                        Person::getName)).limit(10).collect(Collectors.toList());
    }
...
//        return marriedPeopleByAge;
        ...
        return persons.stream().filter(Person::isMarried).
